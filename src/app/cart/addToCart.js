...
useEffect(() => {
  ...
  return (
    <div className='mt-16 w-screen h-auto relative m-auto'>
      <div className='xs:flex justify-between w-full '>
        ...
        :
        <div className=" flex flex-col bg-white w-full  xs:w-5/6 m-3 p-4">
            <div className='flex w-full pb-3 justify-between border-b-2'><h1 className='text-4xl'>Shoping Cart</h1></div>
            {
              products.map((item)=>{ 
                return(
            <div key={item.id} className=' mt-2  border-b-2 pb-3   xs:flex  w-full '>
      <div className="imag flex w-full xs:w-1/4 items-center p-10 ">
      <img
      alt={item.id}
      src={item.image}
      ></img>
      ...
      <div className='  text-2xl font-bold '>{item.title.substring(0, 70)}</div>
      <p className=''>{item.description.substring(0, 200)}</p>
      <div className=' flex text-xl gap-5 '>Unit Price <span>${item.price}</span> </div>
    <div className='flex  gap-2 items-center  text-2xl  '><h2>Qty:</h2><button className='text-2xl font-bold border-2 p-2 rounded-lg   ' onClick={()=>{dispatch(dicrimentQuantity(item.id))}} >-</button> <span>{item.quantity}</span> <button className='text-2xl font-bold border-2 p-2 rounded-lg ' onClick={()=>dispatch(incrimentQuantity(item.id))} >+</button></div>
      <button onClick={()=>dispatch(removeItem(item.id))} className='p-3 px-4 hover:bg-yellow-400 mt-2   bg-yellow-500 w-40 rounded-md cursor-pointer'>Remove Item</button>
      </div>
      <div className='flex items-center justify-start xs:justify-end xs:w-1/6 px-4 text-2xl'> Total ${item.price * item.quantity} </div>   
            </div>
            )})
            }
        ...
        <div className="flex flex-col bg-white w-full xs:w-1/6 h-72 xs:m-10">
